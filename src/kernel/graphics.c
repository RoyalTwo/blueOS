...
// While this will change if I write actual GPU drivers, right now we only need
// a single version of this function since all our graphics will be shown just through
...
void draw_square(int x, int y, int length, uint32_t color)
{
    for (int i = 0; i < length; i++)
    {
        ...
        {
            main_screen.fb_ptr[(j) + (i * main_screen.framebuffer->width) + (x) + (y * main_screen.framebuffer->width)] = color;
        }
    }
}
